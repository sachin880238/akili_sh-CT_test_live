...
var BasicController = AbstractController.extend(FieldManagerMixin, {
    ...
    canBeDiscarded: function (recordID) {
        ...
        var self=this
        var breadcum_tag=$(event.target).parent()
        console.log(",,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,",$(event.target),$(event.target))
        if (breadcum_tag.length >0){
        if(breadcum_tag[0].classList.contains("breadcrumb-item") || event.target.classList.contains("o_pager_previous") || event.target.classList.contains("o_pager_next")){
            // var def = $.Deferred();
        console.log("--------------------------------------<<<<")
        ...
        // //     title: _t("Warning"),
        // //     confirm_callback: def.resolve.bind(def, true),
        // //     cancel_callback:def.resolve.bind(def, false),
        // // });
        // // dialog.on('closed', def, def.reject);
        // // def.done("add")
