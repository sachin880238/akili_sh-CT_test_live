odoo.define('dashboard_custom.favorite_menu', function (require) {
"use strict";
...
FavoriteMenu.include({
    ...
    _addDashboard: function () {
        var self = this;
        var data=window.location.href.split('menu_id=')[1].split('&')[0]
        console.log("fffffffffffffffffffffffffffffffff",this,data)
        var menuitem_id = data
        var modal_name = this.action.env['modelName']
        console.log("fffffffffffffffffffffffffffffffff",modal_name)
        ...
        var xml_id = this.action.xml_id;

        //console.log("-==------------------",search_data,context,domain)
        ...
        var menu_name =$(".o_menu_brand")
        var return_data =this._rpc({
            model: 'dashboard.tile',
            method: 'get_details_wizard',
            args:[currentAction.id || false, c,domain,controller.viewType,name,menu_name[0].innerHTML,modal_name,menuitem_id,xml_id]
        })  .then(function (r) {
                    if(alert(_.str.sprintf(_t("'%s' added to the dashboard"), name))){}
