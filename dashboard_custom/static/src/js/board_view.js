odoo.define('dashboard_custom.BoardView', function (require) {
"use strict";
...
var BoardRenderer = FormRenderer.extend({
    ...
    events: _.extend({}, FormRenderer.prototype.events, {
        ...
        'click .custom_oe_header_txt': 'load_views',
    }),
    ...
    load_views: function () {
       var self=this
       var clickedBtnID = $(event.target);
       var action_id = clickedBtnID[0].getElementsByClassName('action_id')
       var domain = clickedBtnID[0].getElementsByClassName('domain')
       var context = clickedBtnID[0].getElementsByClassName('context')
       //console.log("-=--=------------------",action_id[0].value,domain[0].value,context[0].value)
        // self._rpc({
        //             model: 'ir.actions.actions',
        //             method: 'Check_records',
        //             kwargs: {action_id: action_id[0].value},
        //         }).then(function () {
        //               // self.do_action({
        //               //       name: _t("My Profile"),
        //               //       type: 'ir.actions.act_window',
        ...
        this._rpc({
            model: 'ir.actions.actions',
            method: 'Check_records',
            args: [action_id, {'action_id': action_id[0].value}],
        }).then(function (data) {
            //console.log("-----------------------",data)
            self.do_action({
                name: data['name'],
                type: 'ir.actions.act_window',
                res_model: data['object_name'],
                views: [[false, 'list'], [false, 'form']],
                view_type: "list",
                view_mode: "list",
                // context:domain[0].value,
    ...
    _renderTagBoard: function (node) {
        ...
        //console.log("=-==-==-=-=",hasAction,node.children)
        if (!hasAction) {
        ...
        var menu_name = document.getElementsByClassName("o_menu_brand");
        //console.log("nnnnnnnnnnooooooooooooodddddddddddeeeeeeeee",node,menu_name[0])

        var $html = $('<div>').append($(QWeb.render('DashBoard', {node: node})));
        //console.log("-------===============------=-=--html",$html,this.actionsDescr)
        ...
        //console.log("-----------------===============,,,")
        var j=0;
        if(menu_name.innerHTML == null){
            //console.log("--------------=========================")
            var checkDiv = setInterval(function(){
            
            j=j+1
            //console.log("--------------------=============",j,document.getElementsByClassName("o_menu_brand")[0].innerHTML)
            if(j>50){
                clearInterval(checkDiv);
            }
           if(document.getElementsByClassName("o_menu_brand")[0].innerHTML != "") { 
            //console.log("------=-==-=-=-=-=-==-=s-d=d--ds=ds-",document.getElementsByClassName("o_menu_brand")[0])
               var menu_data= document.getElementsByClassName("o_menu_brand")[0].innerHTML
               var td= $html[0].getElementsByTagName("td");
                var tr = $html[0].getElementsByTagName("tr");
                //console.log("ffffffffiiiiiiiiinnnnnnnnnnal",current_menu[0])
                for(var i = 0; i <td.length; i++) {
                    //console.log("999999999999999999",td[i])
                    var div_data = td[i].getElementsByClassName("oe_action")
                    //console.log("=-=-=-=-==-=-=-=-=-",td[i],div_data)
                    for(var j = 0; j <div_data.length; j++) {
                        var menu_name = div_data[j].getElementsByClassName("menu_name")
                        //console.log("mmmmmmmmmmeeeeeeennnnnnnnnnnuuuu",div_data[j],menu_data)
                        if(menu_data != menu_name[0].innerHTML){
                            div_data[j].style.display="none"
                        }
                        else{
                            var colour_code = div_data[j].getElementsByClassName("colour_dashboard")
                            //console.log("=-=-=colour",colour_code[0].innerHTML)
                            div_data[j].style.background=colour_code[0].innerHTML
                        }
        ...
        var current_menu =$(".o_menu_brand")
        var td= $html[0].getElementsByTagName("td");
        var tr = $html[0].getElementsByTagName("tr");
        //console.log("ffffffffiiiiiiiiinnnnnnnnnnal_saachin",$html[0])
        var count=0
        for(var i = 0; i <td.length; i++) {
            //console.log("999999999999999999",td[i])
            var div_data = td[i].getElementsByClassName("oe_action")
            ...
            for(var j = 0; j <div_data.length; j++) {
                count=count+1
                // var menu_name = div_data[j].getElementsByClassName("menu_name")
                var action_id = div_data[j].getElementsByClassName('action_id')
               var domain = div_data[j].getElementsByClassName('domain')
               var context = div_data[j].getElementsByClassName('context')
                //console.log("mmmmmmmmmmeeeeeeennnnnnnnnnnuuuudomain",domain)
                this._rpc({
                        model: 'ir.actions.actions',
                        method: 'count_records',
                        args: [action_id, {'action_id': action_id[0].value,'domain': domain[0].value,'serial_no':j}],
                    }).then(function (data) {
                        var td_temp= $html[0].getElementsByTagName("td");
                        //console.log("-----------------------final",data,$html[0],td_temp)
                        for(var k=0; k<td_temp.length; k++){
                            //console.log("=-=-=-==-=-=-=-=-=-kkkk",td_temp[k])
                            var div_data_temp = td_temp[k].getElementsByClassName("oe_action")
                            //console.log("=-=-=-=-==-=-=-=-=-",td[i],div_data)
                            for(var h = 0; h <div_data_temp.length; h++) {
                                if(h==data['serial_no']){
                                    var count_number=div_data_temp[h].getElementsByClassName('count_records')
                                    //console.log("-=-=--=-==-----------------------final",div_data_temp[h],count_number)
                                    count_number[0].innerHTML=data['length_records']
                                }
                            }
                        }
...
odoo.define('dashboard_custom.viewRegistry', function (require) {
"use strict";

var BoardView = require('dashboard_custom.BoardView');
