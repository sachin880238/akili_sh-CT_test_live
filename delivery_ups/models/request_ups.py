...
class UPSRequest():
    ...
    def _add_security_header(self, client):
        ...
        ns_security = ('upss', 'http://www.ups.com/XMLSchema/XOLTWS/UPSS/v1.0')
        security = Element('UPSSecurity', ns=ns_security)

        token_username = Element('UsernameToken', ns=ns_security)
        username = Element('Username', ns=ns_security).setText(self.username)
        password = Element('Password', ns=ns_security).setText(self.password)
        token_username.append(username)
        token_username.append(password)

        token_service = Element('ServiceAccessToken', ns=ns_security)
        license = Element('AccessLicenseNumber', ns=ns_security).setText(self.access_number)
        token_service.append(license)

        security.append(token_username)
        security.append(token_service)
