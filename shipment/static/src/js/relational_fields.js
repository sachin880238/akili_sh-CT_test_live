...
var M2ODialog = Dialog.extend({
    ...
    init: function (parent, name, value) {
        this.name = name;
        this.value = value;
        console.log("ffffffffffffffffffffffffffffff",this.name,this.value)
        this.value=null
...
var FieldMany2One = AbstractField.extend({
    ...
    */
    ...
    _search: function (search_val) {
        ...
        // if(self.field.relation == "res.partner"){
        //     this.limit=3
        // }
        this._rpc({
            model: this.field.relation,
            ...
            .then(function (result) {
                ...
                // search more... if more results than limit
                    ...
                    if (!self.attrs.search_more) {
                        values.unshift({
                            label: (self.can_create ? _t("search or create") : _t("search")),
                            action: function () {
                                self._rpc({
                ...
                // create and edit ...
                if (create_enabled && !self.nodeOptions.no_create_edit && values.length == 0) {
    ...
    _onInputFocusout: function () {
        ...
        if(this.field.relation == "res.partner" && specific_record.includes(this.attrs.name)){
        }
        else{
        if (this.can_create && this.floating) {console.log("----------------------------------------------")
...
var FieldX2Many = AbstractField.extend({
    ...
    _renderButtons: function () {
        ...
        } else if (!this.isReadonly && this.view.arch.tag === 'tree') {
            this.$buttons = $(qweb.render('X2Many.buttons', {
                btnClass: 'btn-primary',
                create_text: this.nodeOptions.create_text,
            }));
            this.$buttons.on('click', 'button.o_field_x2many_list_row_add', this._onAddRecord.bind(this));
            this.$buttons.on('click', 'button.o_field_x2many_list_search', this._onSearchRecord.bind(this));
        }
    },
    /**
    ...
    _onSearchRecord: function (ev) {
        var self = this;
        var data = ev.data || {};
        // we don't want interference with the components upstream.
        ev.stopPropagation();
        var domain = self.record.getDomain({fieldName: self.name})
        domain.push([self.field.relation_field, '=', self.record.data.id])
        return new dialogs.SelectCreateDialog(self, _.extend({}, self.nodeOptions, {
            res_model: self.field.relation,
            domain: domain,
            context: _.extend({}, self.record.getContext(self.recordParams), data.context || {}),
            title: (_t("Search: ")) + self.string,
            initial_ids: undefined,
            initial_view: 'search',
            disable_multiple_selection: true,
            no_create: !self.can_create,
            // on_selected: function (records) {
            //     self._openFormDialog({
...
var FieldStatus = AbstractField.extend({
    ...
    supportedFieldTypes: ['selection', 'many2one','many2many'],
    /**
        ...
        if (this.field.type === 'many2one') {
            ...
            self._rpc({
                ...
                kwargs: {
                    ...
                    var result_data=[]
                    var selected_value=[]
                    for(var j=0;j<self.value.res_ids.length;j++){
                            selected_value.push(self.value.res_ids[j])


                        }
                    for(var i=0;i<result.length;i++)
                    {
                        if(selected_value.includes(result[i][0])){
                            result_data.push({selected: true, id: result[i][0], display_name: result[i][1], fold: false})

                        }
                        else{

                        
                        result_data.push({selected: false, id: result[i][0], display_name: result[i][1], fold: false})
                    }

                    }
                    var data=_.map(result_data, function (info) {
    ...
    _render: function () {
        ...
        self._rpc({
                ...
                kwargs: {
                    ...
                    var selections = _.partition(data, function (info) {
            
            return (info.selected || !info.fold);
        });
        self.$el.html(qweb.render("FieldStatus.content", {
